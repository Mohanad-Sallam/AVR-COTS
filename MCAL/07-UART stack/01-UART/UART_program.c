...
void UART_voidInitialize (void)
{
    ...
    URRL= BaudRateValue[UART_U8_SYS_FREQ][UART_U8_BAUD_RATE];
}
u8 UART_u8TransmitDataSynch (u8 Copy_u8Data)
{
    ...
    while ((!GET_BIT(USCRA,6))&&Local_u32TimeOut < UART_U32_TIME_OUT_THRESHOLD)
    {
      Local_u32TimeOut ++;
    }
    /*Time out happened*/
...
u8 UART_u8ReceiveDataSynch (u8 * Copy_pu8Data)
{
    ...
    /*wait till flag is raised*/
    while ((!GET_BIT(USCRA,7))&&Local_u32TimeOut < UART_U32_TIME_OUT_THRESHOLD)
    {
      Local_u32TimeOut ++;
    }
    /*Time out happened*/
